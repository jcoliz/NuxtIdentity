...
- If no `IUserNotifier` is registered, throws `InvalidOperationException` — this is a developer configuration error, not a user error, so it surfaces as 500 via the standard exception middleware
...
The `IUserNotifier` is intentionally not registered automatically. The consumer is responsible for providing their own implementation (email service, SMS gateway, etc.). If no implementation is registered and a user hits the forgot-password endpoint, an `InvalidOperationException` is thrown — making the misconfiguration immediately visible to the developer rather than silently discarding reset codes.
